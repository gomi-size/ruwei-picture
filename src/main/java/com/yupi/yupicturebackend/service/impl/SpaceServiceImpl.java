...
*/
    ...
    public Boolean editSpace(SpaceEditRequest spaceEditRequest, HttpServletRequest request) {
        ...
        //如果已经是该空间类型则不能修改
        if(olspace.getSpaceType().equals(spaceEditRequest.getSpaceType())){
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"已经是该空间类型不需要修改了");
        }

        //设置修改时间
        space.setEditTime(new Date());
        String userIdStr=String.valueOf(loginUser.getId()).intern();
        String lock="project:space:edit:lock:"+userIdStr;
        synchronized (lock) {
            Boolean save=  transactionTemplate.execute(status -> {
                //操作数据库
                boolean result = updateById(space);
                ...
                if (SpaceTypeEnum.TEAM.getValue() == space.getSpaceType()) {
                    ...
                    ThrowUtils.throwIf(!IsSave, ErrorCode.OPERATION_ERROR, "创建团队成员失败");
                }
                //变成了私有空间则需要删除掉记录
                else {
                    LambdaQueryWrapper<SpaceUser> lambdaWrapper = new LambdaQueryWrapper<>();
                    lambdaWrapper.eq(SpaceUser::getSpaceId, space.getId());
                    boolean isRemoved = spaceUserService.remove(lambdaWrapper);
                    ThrowUtils.throwIf(!isRemoved, ErrorCode.OPERATION_ERROR, "删除团队成员失败");
                }
                ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR);
                return result;
            });
